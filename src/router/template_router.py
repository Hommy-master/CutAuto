...
本模块实现了基于模板的草稿创建 RESTful API 接口，包括：
1. 获取模板列表
2. 获取模板详情
3. 使用指定模板创建草稿（支持 688001、688002、688003 等模板）
4. 统一的错误处理和响应格式
"""
...
async def create_draft_with_template(
    ...
    try:
        ...
        # 3. 创建草稿
        result = create_draft(template_id, validated_params)
...
async def create_draft_688002(
    ...
    try:
        result = create_draft("688002", params)
...
async def create_draft_688003(
    ...
    try:
        result = create_draft("688003", params)
